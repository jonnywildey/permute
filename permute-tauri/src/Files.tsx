...
export const Files = memo(
  ({ files, addFiles, removeFile, clearAllFiles, showFile }: IFilesProps) => {
    ...
    const handleRemove = useCallback((path: string) => removeFile(path), [removeFile]);
    const handleShow = useCallback((path: string) => showFile(path), [showFile]);
    const handlePlay = useCallback((file: IPermutationInput) => playFile(file), [playFile]);
