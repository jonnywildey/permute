import { Box, Button, GridItem, Heading, Center, IconButton, PropsOf, Image, Text, List, ListItem, Tooltip, background } from "@chakra-ui/react";
...
export const Output: React.FC<IOutputProps> = ({ output, showFile, setOutput, permutationOutputs }) => {
  ...
  const outputBoxes = permutationOutputs
    ...
    .map((file, i) => {
      ...
      <Tooltip openDelay={200} label={
        <List>
          {file.processors.map( (p, i) => <ListItem>{i + 1}: {p}</ListItem>)}
        </List>
      }>
        <Box 
          width="100%"
  ...
  >
    ...
    <Center>
      <Button mt={3} bg={buttonBg} width="75%" onClick={setOutput}           
      color="gray.50" 
          fontSize="lg"
          cursor="pointer" shadow="base">Select Output Directory</Button>
    </Center>
    {directory}
